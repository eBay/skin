...
function getId(filePath, baseDir) {
  ...
  fileName = fileName.toLowerCase()
    ...
    .replace(/^(ic|icons)-(action|content|programs|status|navigation|status|reviews|filter|device)-/, 'icon-');
...
async function runImport(directory) {
  ...
  const promiseData = await Promise.all(svgs.map(async (filePath) => {
...
require('yargs') // eslint-disable-line
  ...
  .command('dir <directory>', 'generates SVG image file from all SVGs in a given directory', (yargs) => {
    yargs
  ...
  .command('file <file>', 'generates SVG images from a sketch file', (yargs) => {
    yargs
      .positional('file', {
